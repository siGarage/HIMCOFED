  ...
  return (
    <div className="Navbar">
        ...
        <div className='search' style={{width:'230px'}}>
         {A===0 && <input type='text' onChange={handleOnChange}/>}
         <div onClick={()=>handelClick(text)}><div onClick={show_hide1} className='search-button' style={{backgroundColor:'#B80000',color:'white',display:'flex',justifyContent:'center',alignItems:'center',width:'38px',height:'30px'}}> <span className="material-symbols-outlined">search</span></div></div>
        </div>
        </div>
        </div>
  );
